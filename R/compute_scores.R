...
stages      <- read_csv(glue("data/processed/stages/stages_{event_id}_{event_year}.csv"))
results     <- read_csv(glue("data/processed/results/{event_id}_{event_year}_all_stage_results.csv"))
selections  <- read_csv(glue("data/processed/selections/selections_{event_id}_{event_year}.csv"))
...
if ("final" %in% unique(all_participant_stage_scores$stage_id)) {
  stage_lookup <- bind_rows(stage_lookup,
...
participant_stage_ranked <- all_combinations %>%
  left_join(all_participant_stage_scores %>% 
